...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleJoinChat(@ConnectedSocket() client: Socket, @MessageBody('chatIds') chatIds: string[]) {
    ...
    async handleSendMessage(@ConnectedSocket() client: Socket, @MessageBody() body: any) {
        ...
        const message = await this.msgService.sendMessage(userId, body.chatId, sendMsgData);
        this.server.to(`chat:${message.chat.toString()}`).emit('new-message', message);
    }
}
