...
export class PostController {
    ...
    async create(@Request() req: any, @UploadedFiles() images?: Express.Multer.File[], @Body() body?: any) {
    ...
    @Get('feed')
    @UseGuards(AuthGuard)
    async getFeed(@Request() req: any, @Query('page') page = 1, @Query('limit') limit = 15) {
    ...
    @Get('author/:authorId')
    @UseGuards(AuthGuard)
    async getFeedByAuthor(@Request() req: any, @Param('authorId') authorId: string, @Query('page') page = 1, @Query('limit') limit = 15) {
        const userId = req.user;
    ...
    @Put('like/:postId')
    @UseGuards(AuthGuard)
    async updateLikeCounting(@Param('postId') postId: string, @Request() req: any) {
        const userId = req.user.id;
